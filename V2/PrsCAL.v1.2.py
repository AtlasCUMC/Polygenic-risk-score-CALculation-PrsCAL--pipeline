...
#########
    try: 
        if Method == None:
            ...
            os.system('''python PrsCAL.v1.2.py --help''')
            sys.exit()
    except IOError:
...
#######
    ...
    if (Method=="ldpred"):
        ...
        def ldpred_score(PRS,Pheno,PCA,out):
            command = 'Rscript'
            path2script='make_file_bin.v2.R'
            args1=PRS
            args2=Pheno
            args3=PCA
            args4=out
            ...
            command = 'Rscript'
            path2script='PRS_ROC.v2.R'

            args1='''PRS_Bins_qunatiles_'''+out+''''''
            args2='''PRS_ROC_'''+out+''''''
            args3=74
            args4=1740
            cmd=[command,path2script,args1,args2,str(args3),str(args4)]
            subprocess.call(cmd,universal_newlines=True)
            #os.system('''Rscript PRS_ROC.v4.R PRS_Bins_qunatiles_'''+out+''' PRS_ROC_'''+out+''' 1 1 ''')
            
            ######### Top Percentyile####

            os.system('''bash TOP_OR.v2.sh PRS_Bins_qunatiles_'''+out+''' '''+out+'''_Top20 20 19 18 17 ''')
            os.system('''bash TOP_OR.v2.sh PRS_Bins_qunatiles_'''+out+''' '''+out+'''_Top10 20 19 ''')
            os.system('''bash TOP_OR.v2.sh PRS_Bins_qunatiles_'''+out+''' '''+out+'''_Top5 20 20 ''')
            os.system('''bash TOP_OR.v2.sh PRS_Bins_qunatiles_'''+out+''' '''+out+'''_Top2 100 99 100 100 100 100 ''')
            os.system('''bash TOP_OR.v2.sh PRS_Bins_qunatiles_'''+out+''' '''+out+'''_Top1 100 ''')
