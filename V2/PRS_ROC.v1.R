...
reg <-glm(Pheno~PRS+Age+Sex+PC1+PC2+PC3+PC4+T2DM, data=Ldpred_validate, family=binomial(link="logit"))
...
Sroc <-lm(PRS~Age+Sex+PC1+PC2+PC3+PC4+T2DM, data=Ldpred_validate)

GLM <- glm(Pheno~Age+PC1+PC2+PC3+PC4+PRS+T2DM, family=binomial(link="logit"), data=Ldpred_validate)
...
fit <- glm(Pheno~PRS+PC1+PC2+PC3+PC4+Age+Sex+T2DM,data=Ldpred_validate,family=binomial(link="logit"))
...
model = glm(Pheno~PRS+PC1+PC2+PC3+PC4+Age+Sex+T2DM, data=data.frame(g_data2),family = binomial(link="logit"))
...
model = glm(Pheno~PC1+PC2+PC3+PC4+Age+Sex+T2DM, data=data.frame(g_data2),family = binomial(link="logit"))
